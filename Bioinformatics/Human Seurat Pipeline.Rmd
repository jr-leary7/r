...
runname <- "eaton96"
samplename <- "Human_P190722T1Pi"
path2 <- paste0(path1, runname, "/hm38/", samplename, "/raw_feature_bc_matrix")
...
Here we visually compare different QC metrics, including the percentage of mitochondrial DNA that we just added. We can see that this sample has many cells with a high percentage of mitochondrial DNA. 
```{r mito_viz}
...
## PCA
When running PCA, we'll use only the 2000 highly variable genes we identified earlier. This speeds up the process, and ensures that we're capturing sources of variation that are potentially biologically interesting. The visualization techniques below help us to determine sources of heterogeneity in our data. Particularly interesting is the "T" shape seen in the PCA plot. This can be interpreted in several ways. Perhaps one cell type has lower counts than the others, or it could be just a biological difference between cell types. Whatever the root cause, if PCA can separate the data as well as this, there will surely be interesting clusters present in the t-SNE / UMAP / PHATE plots.
...
```{r tsne}
sc <- RunTSNE(sc, reduction = "pca", seed.use = 629, dims = 1:20, dim.embed = 2)
```
...
sc <- RunUMAP(sc, reduction = "pca", dims = 1:20, umap.method = "uwot", 
...
phate_df <- phate(pca_df[, 1:20], ndim = 2, mds.solver = "smacof", npca = 20, seed = 629)
...
sc <- FindClusters(sc, resolution = 0.75)
```

## Visualization
After performing clustering, we can use the dimension reduction results we produced earlier to visualize our clusters. We can see that PCA actually does a decent job separating the clusters for once, and that as predicted one of the ends of the "T" shape is its own cluster.
```{r pca_plot}
DimPlot(sc, reduction = "pca", dims = c(1, 2))
```

t-SNE does a better job than PCA, but the clusters in the middle are still a bit muddled.
```{r tsne_plot}
DimPlot(sc, reduction = "tsne", dims = c(1, 2))
```

We can see here that UMAP did a good job of separating all the clusters, and it appears to me that more of the global structure has been preserved in the form of the distances between the clusters.
```{r umap_plot}
DimPlot(sc, reduction = "umap", dims = c(1, 2))
```

PHATE is used to both separate clusters while preserving global and local structure and identify biological processes in the form of trajectories. We can see some interesting trajectories in almost all of the clusters, including parallel ones in clusters 3 and 4.
```{r phate_plot}
...
saveRDS(sc, file = paste0("/Volumes/Home/Jen Jen Yeh Lab/Jack/scRNAseq/Seurat/RData Files/",
                          samplename, "_MT5_hm38.Rds"))
sc <- readRDS(paste0("/Volumes/Home/Jen Jen Yeh Lab/Jack/scRNAseq/Seurat/RData Files/",
                          samplename, "_MT5_hm38.Rds"))
```
...
for (i in seq(nrow(top_markers))) {
  print(FeaturePlot(sc, features = top_markers$gene[i], dims = c(1, 2)))
}
```
...
for (i in seq(nrow(sc_ref))) {
  rownames(sc_ref)[i] <- strsplit(rownames(sc_ref)[i], "__")[[1]][1]
}
...
The single cell reference did not perform well, assigning an "unclear" label to our epithelial and T cell clusters. I think the structure revealed in the PHATE plot is interesting, specifically the orthogonality of the epithelial and mesenchymal clusters. Also, in all the plots, the epithelial and T cells are placed closely together. This implies that they are proximal to one another in the high dimensional space as well.
```{r, message=FALSE, warning=FALSE}
